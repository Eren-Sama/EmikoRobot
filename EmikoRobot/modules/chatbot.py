...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Giyu AI Already Activated In This Chat")
            return
        await lel.edit(f"Giyu AI Actived by {message.from_user.mention()} for users in {message.chat.title}")

    elif status == "OFF" or status == "off" or status == "Off":
        lel = await edit_or_reply(message, "`Processing...`")
        Escobar = remove_chat(int(message.chat.id))
        if not Escobar:
            await lel.edit("Giyu AI Was Not Activated In This Chat")
            return
        await lel.edit(f"Giyu AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}")
...
__help__ = """
❂ Giyu AI is the only ai system which can detect & reply upto 200 language's
